...
export default function BewertungContent() {
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <div className="mb-8">
          ...
          <p className="text-sm text-slate-600 mt-2">Modul {currentModule} von {MODULES.length}</p>
        </div>
              ...
              {MODULES.map((mod) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {mod.number < currentModule && <CheckCircle className="w-4 h-4 text-emerald-600" />}
                  </div>
        ...
        <div className="flex justify-between mt-8">
          <Button variant="outline" disabled={currentModule === 1} onClick={() => setCurrentModule(currentModule - 1)}>
            <ChevronLeft className="w-4 h-4 mr-2" />Zurück
          </Button>
          <Button disabled={currentModule === MODULES.length} onClick={() => setCurrentModule(currentModule + 1)}>
